...
from app.models.models import RegionRate, ShippingZone, TaxRate, PostalCode, CountryCallingCode
...
from app.schemas.shipping import (
    RegionRateCreate, RegionRateRead,
    ShippingZoneCreate, ShippingZoneRead,
...
@router.post("/region-rates/", response_model=RegionRateRead)
def create_region_rate(rr: RegionRateCreate, db: Session = Depends(get_db)):
    rr_obj = RegionRate(**rr.dict())
    return create_region_rate(db, rr_obj)

@router.get("/region-rates/", response_model=list[RegionRateRead])
def read_region_rates(db: Session = Depends(get_db)):
    return get_region_rates(db)


@router.post("/shipping-zones/", response_model=ShippingZoneRead)
def create_shipping_zone(sz: ShippingZoneCreate, db: Session = Depends(get_db)):
    sz_obj = ShippingZone(**sz.dict())
    return create_shipping_zone(db, sz_obj)

@router.get("/shipping-zones/", response_model=list[ShippingZoneRead])
def read_shipping_zones(db: Session = Depends(get_db)):
    return get_shipping_zones(db)
